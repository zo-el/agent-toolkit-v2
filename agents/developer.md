...
- Spawn `pr-review-toolkit` agents on your committed range, `<base>..<head>` by sha, for the lenses that fit: `silent-failure-hunter` for error handling, `type-design-analyzer` for types, `pr-test-analyzer` for coverage, `comment-analyzer` whenever the change adds or edits comments, `code-reviewer` for a general read. Run `/security-review` on anything touching credentials, auth, parsing, crypto, or network trust.
...
- `Reuse:` one line naming what you searched for before adding a new helper, type, or dependency, and what you found. A new dependency names its health: last release, maintenance, adoption. `Reuse: none needed` when the change added none of these.
