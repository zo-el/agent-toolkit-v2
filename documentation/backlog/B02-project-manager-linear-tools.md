...
**Priority:** P1
**Target:** agent-toolkit-v2: agents/project-manager.md, tests/install.sh
...
Check first whether `ToolSearch`, which the definition also declares, already reaches the Linear tools: a harness that surfaces an MCP server's tools by name for `ToolSearch` to load would make the bare prefixes harmless and settle this without touching the definition.
