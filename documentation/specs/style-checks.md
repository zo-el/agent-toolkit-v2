...
One block, listing every finding at once, each on its own line with its `path:line` and the offending text. An agent fixes all of them in one pass rather than discovering them one at a time. Past `FINDINGS_SHOWN` the rest are counted and not listed, and the count says only how many: a truncated report is a mixed set, so it claims nothing about their kind. A commit's comment drift finding leads its own findings, because there is one of it and the cut would otherwise take it. Where one command carries several commits, each commit's findings follow the last one's. The block closes with what each rule asks for, and with the override and what it is for.
...
| `COMMENT_NET` | 20 | past the largest drift read and found earned, and the low end of the 20 to 25 band this repository's history answers identically; 3 capped a commit at 58 changed lines and fired on 45% of it |
