...
- **The seal is checked immediately before `install.sh` starts, and the window it leaves open is the install itself.** The check is a walk of the tree, so a file read early can change before the walk reaches the last one, and install then reads that same tree for the whole of its run, plugin fetches included. What makes that acceptable is not the size of the window: it is the other half of this, that a release directory is approved for nobody, so no agent writes there without the user being asked.
...
- **Sealing the live version directory too.** It would fail on a healthy machine. `install.sh` and `hooks/update.sh` set `PYTHONDONTWRITEBYTECODE` for themselves and `hooks/launcher.sh` does not, so the first status line render writes `hooks/lib/__pycache__` into the live tree and it has drifted from any seal within seconds. That cache is what keeps a hook running on every tool call cheap, so the answer is not to stop the drift. The argument holds underneath it anyway: a staged tree has one moment before it runs and a live one has none, its hooks having fired already, and a check computed by code inside the tree it is checking is not a control. What covers the live tree is that a release directory is approved for nobody.
