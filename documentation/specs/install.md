...
- **Install refuses a version directory under `~/.claude/worktrees`**, so the one path by which an agent-writable directory becomes the live toolkit is closed rather than argued about. The finding names the checkout the worktree belongs to, which git can answer from the worktree itself.
...
- **Two entries are kept unset rather than merely retired**, and removed at every apply whoever wrote them: `~/.claude` itself, and any release directory. Retirement needs a ledger to vouch that the toolkit wrote a value, and a machine that lost its ledger would otherwise keep, for good, the one approval this rule exists to remove. Setting either again means changing the toolkit.
...
| the root being installed is under `~/.claude/worktrees` | nothing written, exit `1`. A worktree is approved for every agent, so it never becomes the live toolkit. The finding names the checkout it belongs to and the command to install from there |
